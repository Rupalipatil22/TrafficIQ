...
export default function VehicleTracking() {
  ...
  const search = (targetPlate) => {
    ...
    fetch(`/api/vehicles/${targetPlate.trim()}/trajectory`)
      .then((res) => {
        if (!res.ok) throw new Error('No trajectory found in PostgreSQL database');
        return res.json();
      })
    ...
    <div style={{ padding: '24px 32px' }}>
      <div style={{ display: 'flex', gap: '12px', marginBottom: '20px' }}>
        <input
          ...
          style={{ padding: '10px 14px', borderRadius: '6px', background: '#131e3a', color: '#fff', border: '1px solid #334155', outline: 'none', width: '280px' }}
        />
        <button onClick={() => search(plate)} style={{ padding: '10px 18px', background: '#2563eb', color: '#fff', border: 'none', borderRadius: '6px', fontWeight: '600' }}>
          Reconstruct Route
        </button>
      </div>

      {error && <p style={{ color: '#ef4444', marginBottom: '14px', fontSize: '14px' }}>{error}</p>}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '360px 1fr', gap: '20px', height: '580px' }}>
        <div style={{ backgroundColor: '#131e3a', padding: '20px', borderRadius: '8px', overflowY: 'auto', border: '1px solid #1e293b' }}>
          <h3 style={{ fontSize: '15px', marginBottom: '18px', color: '#e2e8f0' }}>Spatial-Temporal Route: {plate}</h3>
